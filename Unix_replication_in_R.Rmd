...
maize_snp_count <- maize_snp_count |>
  mutate(Chromosome = factor(Chromosome, levels = chromosome_order))

# maize SNPs per chromosome plot
maize_snp_plot <- ggplot(maize_snp_count, aes(x = Chromosome, y = SNP_count, fill = Chromosome)) +
  geom_bar(stat = "identity") +
  labs(
    title = "SNP Count per Chromosome (maize)", 
    x = "Chromosome", 
    y = "SNP Count"
  ) +
  theme_minimal() +
...
teosinte_snp_count <- teosinte_join |>
  group_by(Chromosome) |>
  summarise(SNP_count = n(), .groups = "drop")

# ensure Chromosome's are in correct order
teosinte_snp_count < teosinte_snp_count |>
  mutate(Chromosome = factor(Chromosome, levels = chromosome_order))

# teosinte SNPs per chromosome plot
teosinte_snp_plot <- ggplot(teosinte_snp_count, aes(x = Chromosome, y = SNP_count, fill = Chromosome)) +
  geom_bar(stat = "identity") +
  labs(
    title = "SNP Count per Chromosome (teosinte)", 
    x = "Chromosome", 
    y = "SNP Count"
  ) +
  theme_minimal() +
...
combined_snp_plot <- 
  ...
  plot_annotation(
    title = "SNP Count per Chromosome", 
    theme = theme(
