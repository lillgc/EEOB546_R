...
process_chr_data <- function(data, chr_num, output_prefix, replace_na, sort_order = "asc", output_dir = "processed_data") {
  ...
  filtered_data <- data %>%
    ...
    mutate(across(everything(), ~ gsub("\\?/\\?", replace_na, .))) %>%
    arrange(ifelse(sort_order == "asc", Position, desc(Position)))
...
```{r}
# ~~~ maize ascending ~~~
process_chr_data(maize_join, 1, "maize", "?/?", "asc", output_dir = "maize_data")
maize_chr1_asc_check <- read_table("maize_data/maize_chr1_asc.txt")

# ~~~ maize descending ~~~
process_chr_data(maize_join, 1, "maize", "-/-", "desc", "maize_data") 
